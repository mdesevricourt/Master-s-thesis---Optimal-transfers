...
using utilityfunctions, Searcheffort
export Welfare, budgetcons, budget, τofT, netwage
...
function τofT(T::Real; gw::Real = Default["wage"], ρ::Real = Default["tax rate"], δ::Real = Default["home production"], p_e_e = Default["p_e|e"], p_u_u = Default["p_u|u"], λ::Real  = Default["Market tightness"])
    ...
    return([τ, Optimeffort(T, τ; wb = gw, wn = nw , δ = δ, p_e_e = p_e_e, p_u_u = p_u_u, λ = λ), budget(ρ = ρ, gw = gw, e = e, p_e_e = p_e_e)])
end
